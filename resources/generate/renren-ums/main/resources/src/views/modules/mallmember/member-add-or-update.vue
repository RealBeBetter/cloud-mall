<template>
  <el-dialog
    ...
    <el-form-item label="社交账号ID" prop="socialUid">
      <el-input v-model="dataForm.socialUid" placeholder="社交账号ID"></el-input>
    </el-form-item>
    <el-form-item label="社交账号Token" prop="accessToken">
      <el-input v-model="dataForm.accessToken" placeholder="社交账号Token"></el-input>
    </el-form-item>
    <el-form-item label="社交账号Token过期时间" prop="expiresIn">
      <el-input v-model="dataForm.expiresIn" placeholder="社交账号Token过期时间"></el-input>
    </el-form-item>
    </el-form>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          socialUid: [
            { required: true, message: '社交账号ID不能为空', trigger: 'blur' }
          ],
          accessToken: [
            { required: true, message: '社交账号Token不能为空', trigger: 'blur' }
          ],
          expiresIn: [
            { required: true, message: '社交账号Token过期时间不能为空', trigger: 'blur' }
          ]
        }
      }
    },
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/mallmember/member/info/${this.dataForm.id}`),
              method: 'get',
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/mallmember/member/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
