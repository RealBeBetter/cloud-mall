<template>
  <el-dialog
    ...
    <el-form-item label="JSON" prop="content">
      <el-input v-model="dataForm.content" placeholder="JSON"></el-input>
    </el-form-item>
    ...
    <el-form-item label="0-新建 1-已发送 2-错误抵达 3-已抵达" prop="messageStatus">
      <el-input v-model="dataForm.messageStatus" placeholder="0-新建 1-已发送 2-错误抵达 3-已抵达"></el-input>
    </el-form-item>
  ...
  export default {
    data () {
      return {
        ...
        dataRule: {
          content: [
            { required: true, message: 'JSON不能为空', trigger: 'blur' }
          ],
          toExchange: [
          ...
          messageStatus: [
            { required: true, message: '0-新建 1-已发送 2-错误抵达 3-已抵达不能为空', trigger: 'blur' }
          ],
          createTime: [
            { required: true, message: '不能为空', trigger: 'blur' }
          ],
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.messageId) {
            this.$http({
              url: this.$http.adornUrl(`/mallorder/mqmessage/info/${this.dataForm.messageId}`),
              method: 'get',
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/mallorder/mqmessage/${!this.dataForm.messageId ? 'save' : 'update'}`),
              method: 'post',
